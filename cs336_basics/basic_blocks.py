...
class Embedding(torch.nn.Module):
    embeds: Float[torch.Tensor, "vocab_size d_model"]
    def __init__(self, num_embeddings: int, embedding_dim: int, device: torch.device | None = None, dtype: torch.dtype | None = None):
        ...
        """
        super().__init__()
        self.embeds = torch.nn.Parameter(torch.empty(num_embeddings, embedding_dim))
        torch.nn.init.trunc_normal_(self.embeds,mean=0, std=1, a = -3, b = 3)
